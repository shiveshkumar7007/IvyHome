...
A React + Vite real estate intelligence platform built on the Ivy Homes Property API. The project treats the initial API\_REFERENCE.md as a hypothesis rather than a source of truth, and validates documented behavior against the running API the results of that validation live in **submission.json** at the repo root, alongside every number the assignment asks for and every place the docs turned out to be wrong.
...
The audit script (analyze.js, repo root): This is what produces every value in submission.json's answers and most of the evidence in findings.
...
Create a .env file in the repo root as shown below:



**Code snippet**

BASE\_URL=https://solve.ivy.homes

API\_KEY=IVY26-0328057EAE2B

DEMO\_PASSWORD=1926030057



Note on data files: While the raw JSON snapshots (listings.json, rentals.json, projects.json) are git-ignored at the repo root data/ folder for the audit script, they are also bundled inside frontend/public/ for seamless client-side web deployment.



Run the audit script locally after fetching or populating the data snapshots:
...
* listing\_id uniqueness. The docs claim every listing\_id is globally unique, and across all 3,800 records it is — the duplication in this dataset is at the property level (same coordinates, different source-site listing\_ids), which is a separate, correctly documented thing, not a contradiction of this claim.
...
* Resolve the posted\_at timezone question with a live check rather than an inference — post a test listing, read back its posted\_at, compare to wall-clock time, and confirm whether listings\_last\_7\_days should be 128 (treating the value as already IST) or 115 (treating it as UTC without the suffix).
