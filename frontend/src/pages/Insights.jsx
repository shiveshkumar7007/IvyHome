...
export default function Insights() {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        if (!response.ok) throw new Error("Failed to fetch listings.json");
        
        const jsonData = await response.json();
        const items = jsonData.results || jsonData.data || jsonData.listings || jsonData || [];
        setRawListings(items.map(fixPropertyData));
      } catch (err) {
  ...
  const auditData = useMemo(() => {
    ...
    rawListings.forEach((l) => {
      ...
      const isNegative = l.price < 0;
      const isBadArea = l.carpet_area > 0 && l.super_built_up_area > 0 && l.carpet_area > l.super_built_up_area;
      const isBadFloor = l.total_floors > 0 && l.floor > l.total_floors;
      ...
      if (l.listing_id && l.listing_id.startsWith("MAG-")) {
  ...
  const filteredMetrics = useMemo(() => {
    if (!rawListings.length) return null;
    ...
    const validPrices = activeListings.filter((l) => l.price > 0).map((l) => Number(l.price)).sort((a, b) => a - b);
    ...
    const avg2BHKPriceSqft = valid2BHK.length === 0 ? 0 : valid2BHK.reduce((sum, l) => sum + (l.price / l.carpet_area), 0) / valid2BHK.length;
    ...
    const validAreas = dataset.filter((l) => l.carpet_area > 0).map((l) => l.carpet_area);
    ...
    return {
      totalCount: dataset.length, activeCount: activeListings.length, medianPrice, avg2BHKPriceSqft: Math.round(avg2BHKPriceSqft),
      verifiedCount, ownerCount, avgArea: Math.round(avgArea), bhkDistribution, propertyTypes
    };
        ...
        <div className="mb-8 flex flex-col md:flex-row md:items-end justify-between gap-4 border-b border-[#1E2022]/10 pb-6">
          <div>
            ...
            <p className="mt-1 text-sm text-[#1E2022]/60">
              Interactive analytics engine with real-time audit tracing.
            </p>
          </div>
